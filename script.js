...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.querySelector('.pet-card')) {
        ...
        const proceedBtn = document.getElementById('proceed-to-adopt');
        
        // Get all adopt buttons
        const adoptButtons = document.querySelectorAll('.btn-buy');
        
        adoptButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                e.preventDefault();
                
                // Get pet info from the card
                ...
                // Set the pet parameter for the adoption form
                const petParam = petName.toLowerCase().split(' ')[0];
                proceedBtn.href = `adopt.html?pet=${petParam}`;
